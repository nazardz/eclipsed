...
mod.GrabItemCallback:AddCallback(mod.GrabItemCallback.InventoryCallback.POST_ADD_ITEM, function (player, item, count, touched, fromQueue)
	if not touched or not fromQueue then
		...
		for _, fam in pairs(fams) do
			if datatables.MongoCells.HiddenWispEffectsVariant[fam.Variant] then
				local newItem = datatables.MongoCells.HiddenWispEffectsVariant[fam.Variant]
				if not player:HasCollectible(newItem) then
					mod.hiddenItemManager:AddForRoom(player, newItem, -1, 1, "MONGO_CELLS")
				end
			end
			if datatables.MongoCells.FamiliarEffectsVariant[fam.Variant] then
				local newEffect = datatables.MongoCells.FamiliarEffectsVariant[fam.Variant]
				if newEffect[2] then
			...
			if datatables.MongoCells.TrinketFamiliarVariant[fam.Variant] then
				local trinket = datatables.MongoCells.TrinketFamiliarVariant[fam.Variant]
				if not player:HasTrinket(trinket) then
					functions.TrinketAdd(player, trinket)
...
mod.GrabItemCallback:AddCallback(mod.GrabItemCallback.InventoryCallback.POST_ADD_ITEM, function (player, item, count, touched, fromQueue)
	if not touched or not fromQueue then
		...
		player:SetFullHearts()
		player:UsePill(PillEffect.PILLEFFECT_LARGER, 0, datatables.NoAnimNoAnnounMimic | UseFlag.USE_NOHUD)
		player:AnimateHappy()
		local data = player:GetData()
		data.eclipsed = data.eclipsed or {}
...
function mod:onStart(isSave)
	...
	if isSave then
		...
		itemPool:IdentifyPill(enums.Pickups.RedPillColor)
		functions.ResetModVars(true)
...
function mod:onPlayerInit(player)
	...
	if playerType == enums.Characters.Abihu then -- nadab
		...
		if not player:HasCollectible(enums.Items.Ignite, true) then
			player:SetPocketActiveItem(enums.Items.Ignite, ActiveSlot.SLOT_POCKET, true)
		end
	else
		if data.eclipsed.BlindCharacter and playerType ~= enums.Characters.UnbiddenB then
		...
		if data.eclipsed.AbihuCostumeEquipped then
			...
			if player:HasCollectible(enums.Items.Ignite, true) then player:RemoveCollectible(enums.Items.Ignite) end
		end
	end
	---UnbiddenB
	if playerType == enums.Characters.UnbiddenB then
		...
		if data.eclipsed.BlindCharacter and playerType ~= enums.Characters.Abihu then
			data.eclipsed.BlindCharacter = nil
			functions.SetBlindfold(player, false)
		end
		if player:HasCollectible(enums.Items.Threshold, true) then
			player:RemoveCollectible(enums.Items.Threshold)
		end
	end
end
...
function mod:onCache(player, cacheFlag)
	...
	if data.eclipsed then
		...
		if cacheFlag == CacheFlag.CACHE_DAMAGE then
			...
			if player:HasCollectible(enums.Items.SteroidMeat) and data.eclipsed.SteroidDamage then
			...
			if player:HasCollectible(enums.Items.BaconPancakes) then
				player.Damage = player.Damage + datatables.BaconPancakes.Damage
			end
			if player:HasTrinket(enums.Trinkets.PhotocopyPHD) and data.eclipsed.DamagePHD then
		...
		if cacheFlag == CacheFlag.CACHE_FIREDELAY then
			...
			if player:HasCollectible(enums.Items.BaconPancakes) then
				player.MaxFireDelay = player.MaxFireDelay - datatables.BaconPancakes.MaxFireDelay
			end
		end
		if cacheFlag == CacheFlag.CACHE_SHOTSPEED then
			...
			if player:HasCollectible(enums.Items.BaconPancakes) then
				player.ShotSpeed = player.ShotSpeed + datatables.BaconPancakes.ShotSpeed
			end
		end
		if cacheFlag == CacheFlag.CACHE_RANGE then
			if player:HasCollectible(enums.Items.VoidKarma) and data.eclipsed.KarmaStats then
			...
			if player:HasCollectible(enums.Items.BaconPancakes) then
				player.TearRange = player.TearRange + datatables.BaconPancakes.Range
			end
		end
		if cacheFlag == CacheFlag.CACHE_SPEED then
			...
			if player:HasCollectible(enums.Items.BaconPancakes) then
				player.MoveSpeed = player.MoveSpeed + datatables.BaconPancakes.MoveSpeed
			end
		end
		if cacheFlag == CacheFlag.CACHE_TEARCOLOR then
...
function mod:onPlayerTakeDamage(entity, _, flags) --entity, amount, flags, source, countdown
	...
	if player:HasCollectible(enums.Items.MongoCells) and flags & DamageFlag.DAMAGE_NO_PENALTIES == 0 then
		...
		for _, fam in pairs(fams) do
			...
			elseif fam.Variant == enums.Familiars.NadabBrain then
	...
	---HolyRavioli
	if player:HasCollectible(enums.Items.HolyRavioli) then
...
function mod:onPlayerCollision(player, collider)
	...
	---MarkNature
	--[[
	if player:HasTrinket(enums.Trinkets.MarkNature) and collider:ToPickup() then
		local rng = player:GetTrinketRNG()
		local pickup = collider:ToPickup()
		if not pickup:IsShopItem() and rng:RandomFloat() < player:GetTrinketMultiplier(enums.Trinkets.MarkNature) * 0.1 then
...
function mod:onPEffectUpdate(player)
	...
	if data.eclipsed.ForRoom.VHSdelay and game:GetFrameCount() - data.eclipsed.ForRoom.VHSdelay > 30 then
		...
		local rng = player:GetCollectibleRNG(enums.Items.VHSCassette)
	...
	if (playerType == enums.Characters.Nadab or playerType == enums.Characters.Abihu or player:HasCollectible(enums.Items.NadabBody, true)) and data.eclipsed.ExCountdown then
		--data.eclipsed.ExCountdown = data.eclipsed.ExCountdown or 0
	...
	---ExplodingKittens
	if datatables.ExplodingKittens.BombCards[player:GetCard(0)] then
	...
	if player:HasCollectible(enums.Items.Threshold) and game:GetFrameCount()%30 == 0 then -- every 2 seconds
		for slot = 0, 3 do
			if player:GetActiveItem(slot) == enums.Items.Threshold and player:GetActiveCharge(slot) >= Isaac.GetItemConfig():GetCollectible(enums.Items.Threshold).MaxCharges then
				...
				if not data.eclipsed.RenderThresholdItem then --and itemWisps[1].Variant == FamiliarVariant.ITEM_WISP and not functions.CheckItemType(itemWisps[1].SubType) then
	...
	if player:HasCollectible(enums.Items.NadabBody, true) then
		...
		if playerType == enums.Characters.Abihu and Input.IsActionPressed(ButtonAction.ACTION_BOMB, player.ControllerIndex) then
	...
	---Abihu
		...
		if data.eclipsed.ForRoom.AbihuBrimstone and (not data.eclipsed.ForRoom.BrimDealy or game:GetFrameCount()%data.eclipsed.ForRoom.BrimDealy == 0) then
			...
			functions.ShootAbihuFlame(player, player:GetLastDirection()*player.ShotSpeed * 14)
		...
		elseif data.eclipsed.AbihuIgnites then
			...
			local multiShotNum = functions.GetMultiShotNum(player, false)-1
			...
			if player:HasCollectible(CollectibleType.COLLECTIBLE_LUDOVICO_TECHNIQUE) then
				if not data.eclipsed.AbihuLudoFlame or not data.eclipsed.AbihuLudoFlame:Exists() then
					...
					data.eclipsed.AbihuLudoFlame = functions.ShootAbihuFlame(player, player:GetLastDirection())
					data.eclipsed.AbihuLudoFlame:GetData().LudovicoFire = functions.GetItemsCount(player, CollectibleType.COLLECTIBLE_LUDOVICO_TECHNIQUE)-1  + multiShotNum
					...
					if data.eclipsed.AbihuDamageDelay == maxCharge or data.eclipsed.SoyCharge then
						...
						if player:HasCollectible(CollectibleType.COLLECTIBLE_MONSTROS_LUNG) then
							...
							for _ = 1, 14 + multiShotNum do
								...
								functions.ShootAbihuFlame(player, newVelocity, newDamage)
							end
							data.eclipsed.AbihuDamageDelay = 0
						end
	...
	---UnbiddenB
		...
		if (player:HasCollectible(CollectibleType.COLLECTIBLE_TECHNOLOGY) or player:HasCollectible(CollectibleType.COLLECTIBLE_BRIMSTONE) or player:HasCollectible(CollectibleType.COLLECTIBLE_TECH_X)) then
			...
			data.eclipsed.UnbiddenBrimCircle = true
		...
		if data.eclipsed.BlindCharacter or player:HasCollectible(CollectibleType.COLLECTIBLE_LUDOVICO_TECHNIQUE) then
			...
			if player:HasCollectible(CollectibleType.COLLECTIBLE_LUDOVICO_TECHNIQUE) then 
				for _, tear in pairs(Isaac.FindByType(EntityType.ENTITY_TEAR)) do
					...
					if tear:HasTearFlags(TearFlags.TEAR_LUDOVICO) and tear.Scale > 1 and tear.SpawnerEntity and tear.SpawnerEntity:ToPlayer() and tear.SpawnerEntity:ToPlayer():GetPlayerType() == enums.Characters.UnbiddenB then
				...
				if not auraPosLudo then
					for _, tear in pairs(Isaac.FindByType(EntityType.ENTITY_LASER)) do
						...
						if tear.SubType == LaserSubType.LASER_SUBTYPE_RING_LUDOVICO and tear.SpawnerEntity and tear.SpawnerEntity:ToPlayer() and tear.SpawnerEntity:ToPlayer():GetPlayerType() == enums.Characters.UnbiddenB then
							auraPosLudo = tear.Position
							break
						end
					end
			...
			if not player:HasWeaponType(WeaponType.WEAPON_LUDOVICO_TECHNIQUE) and (player:GetFireDirection() == Direction.NO_DIRECTION or data.eclipsed.UnbiddenMarkedAuto) then
				...
				elseif player:HasCollectible(CollectibleType.COLLECTIBLE_EYE_OF_THE_OCCULT) then
					if data.eclipsed.UnbiddenBDamageDelay >= maxCharge then
						local tearsNum = functions.GetMultiShotNum(player)
						data.eclipsed.MultipleAura = data.eclipsed.MultipleAura or 0
						data.eclipsed.MultipleAura = data.eclipsed.MultipleAura + tearsNum
	...
	---Shroomface
	if player:HasCollectible(enums.Items.Shroomface) and game:GetFrameCount()%240 == 0 then
		local rng = player:GetCollectibleRNG(enums.Items.Shroomface)
		for _ = 1, 3 do
	...
	if data.eclipsed.GiftCertificate and game:GetFrameCount() - data.eclipsed.GiftCertificate >= 2 then
		...
		if data.eclipsed.GiftCertificate then
			for _ = 1, player:GetTrinketMultiplier(enums.Trinkets.GiftCertificate) do
				player:UseActiveItem(CollectibleType.COLLECTIBLE_COUPON, datatables.NoAnimNoAnnounMimic)
				player:TryRemoveTrinket(enums.Trinkets.GiftCertificate)
			end
		end
		data.eclipsed.GiftCertificate = nil
	end
	...
	if data.eclipsed.BlackPearl and game:GetFrameCount() - data.eclipsed.BlackPearl >= 2 then
		...
		if data.eclipsed.BlackPearl then
			player:AddBlackHearts(player:GetTrinketMultiplier(enums.Trinkets.BlackPearl))
			player:TryRemoveTrinket(enums.Trinkets.BlackPearl)
		end
		data.eclipsed.BlackPearl = nil
	end
...
function mod:onUpdate()
	...
	for playerNum = 0, game:GetNumPlayers()-1 do
		...
		if player:IsDead() then
			...
			---Nadab
			if plyaerType == enums.Characters.Nadab and player:HasCollectible(CollectibleType.COLLECTIBLE_BIRTHRIGHT, true) then
			...
			if player:GetBrokenHearts() < 11 and (plyaerType == enums.Characters.Unbidden or plyaerType == enums.Characters.UnbiddenB) and not data.eclipsed.NoAnimReset then
				if room:GetType() == RoomType.ROOM_DUNGEON and room:GetRoomConfigStage() == 35 then
			...
			if player:GetExtraLives() < 1 then
				...
				---AngryMeal
				if player:HasCollectible(enums.Items.AngryMeal, true) then
					player:UseCard(Card.CARD_SOUL_LAZARUS, datatables.NoAnimNoAnnounMimic)
					player:RemoveCollectible(enums.Items.AngryMeal)
					player:UseActiveItem(CollectibleType.COLLECTIBLE_BERSERK, datatables.NoAnimNoAnnounMimic)
					player:AddHearts(1)
				end
				---AbyssCart
				if player:HasTrinket(enums.Trinkets.AbyssCart) then
	...
	for playerNum = 0, game:GetNumPlayers()-1 do
		...
		if not player:HasCurseMistEffect() and not player:IsCoopGhost() then
			...
			if player:HasCollectible(enums.Items.MongoCells) then
				...
				local fams = Isaac.FindByType(EntityType.ENTITY_FAMILIAR)
				for _, fam in pairs(fams) do
					if datatables.MongoCells.FlyFamiliarVariant[fam.Variant] then
						player:AddSwarmFlyOrbital(player.Position)
					end
					--[[
					elseif datatables.MongoCells.CleanFamiliarVariant[fam.Variant]  then
						local newPickup = datatables.MongoCells.CleanFamiliarVariant[fam.Variant]
...
function mod:onNewRoom()
	...
	for playerNum = 0, game:GetNumPlayers()-1 do
		...
		functions.ResetPlayerData(player)
		local data = player:GetData()
		if data.eclipsed.BlindCharacter and (playerType == enums.Characters.UnbiddenB or playerType == enums.Characters.Abihu) then
		...
		if not room:IsClear() then
			---Penance
			if player:HasTrinket(enums.Trinkets.Penance) then
			...
			---IvoryTarget
			--[[
			if player:HasTrinket(enums.Trinkets.IvoryTarget) then
				local rng = player:GetTrinketRNG(enums.Trinkets.IvoryTarget)
				for _, entity in pairs(Isaac.GetRoomEntities()) do
					if entity:ToNPC() and entity:IsActiveEnemy() and entity:IsVulnerableEnemy() and not entity:GetData().IvoryTargetMark and rng:RandomFloat() < 0.16 * player:GetTrinketMultiplier(enums.Trinkets.IvoryTarget) then
		...
		if player:HasCollectible(enums.Items.MongoCells) then
			...
			for _, fam in pairs(fams) do
				...
				if datatables.MongoCells.HiddenWispEffectsVariant[fam.Variant] then
					local item = datatables.MongoCells.HiddenWispEffectsVariant[fam.Variant]
				...
				for _, trinketFam in pairs(data.eclipsed.MongoTrinket) do
					if player:HasTrinket(trinketFam[1]) then
						if #Isaac.FindByType(EntityType.ENTITY_FAMILIAR, trinketFam[2]) == 0 then
							functions.TrinketRemove(player, trinketFam[1])
						else
...
function mod:onEnemyInit(enemy)
	if mod.ModVars then
		...
		if not datatables.DeliriumBeggarBan[enemy.Type] and enemy:IsActiveEnemy() and enemy:IsVulnerableEnemy() and not enemy:IsBoss() and not mod.ModVars.ForLevel.SavedEnable[tostring(enemy.Type..enemy.Variant)] then
...
function mod:onEnemyTakeDamage(enemy, _, flags, source)
	...
	local enemyData = enemy:GetData()
	---CurseBishop
	if level:GetCurses() & enums.Curses.Bishop > 0 and not enemy:IsBoss() and rng:RandomFloat() < 0.16 then
	...
	if source.Entity:ToPlayer() then
		...
		if flags & DamageFlag.DAMAGE_LASER == DamageFlag.DAMAGE_LASER then
			...
			---GlitterInjection
			if player:HasCollectible(enums.Items.GlitterInjection) then
				local chance = 1/(30-(functions.LuckCalc(player.Luck, 13)*2))
	...
	if flags & DamageFlag.DAMAGE_EXPLOSION == DamageFlag.DAMAGE_EXPLOSION or flags & DamageFlag.DAMAGE_TNT == DamageFlag.DAMAGE_TNT then
		for playerNum = 0, game:GetNumPlayers()-1 do
			...
			---Pyrophilia
			if ppl:HasCollectible(enums.Items.Pyrophilia) and ppl:CanPickRedHearts() then
				ppl:AddHearts(1)
				sfx:Play(SoundEffect.SOUND_VAMP_GULP)
	...
	if source.Entity:GetData().AbihuFlame then
		...
		functions.AbihuSplit(flame, ppl)
...
function mod:onTearUpdate(tear)
	...
	if tearData.KnifeTear then
		...
		if tear.FrameCount == 1 then
			tear.Visible = true
		elseif tear.FrameCount < 900 then
	...
	---GlitterInjection
	if tear:HasTearFlags(TearFlags.TEAR_LUDOVICO) and player:HasCollectible(enums.Items.GlitterInjection) and tear.FrameCount%15 == 0 then
		local chance = 1/(30-(functions.LuckCalc(player.Luck, 13)*2))
		if chance > rng:RandomFloat() then
	...
	if tear.FrameCount > 1 then return end
	---GlitterInjection
	if not tearData.Glittered and player:HasCollectible(enums.Items.GlitterInjection) and not tear:HasTearFlags(TearFlags.TEAR_LUDOVICO) then
		local chance = 1/(30-(functions.LuckCalc(player.Luck, 13)*2))
		if chance > rng:RandomFloat() then
...
function mod:onLaserUpdate(laser)
	...
	elseif laserData.UnbiddenBrimLaser then 
		laser.Color = Color(1,1,1,1,-0.5,0.7,1)
		if laserData.UnbiddenBrimRange and laserData.UnbiddenBrimRange > 0 then
			laser.Radius = laser.Radius + 5
			--print(laser.Radius*laserData.UnbiddenBrimRange)
			if laser.Radius >= laserData.UnbiddenBrimRange then
				laserData.UnbiddenBrimRange = 0
			end
		else
			laser.Radius = laser.Radius - 5
		end	
...
function mod:onKnifeUpdate(knife)
	...
	--[
	if player:GetPlayerType() == enums.Characters.UnbiddenB and knife:IsFlying() then 
	...
	---GlitterInjection
	if player:HasCollectible(enums.Items.GlitterInjection) and knife.FrameCount%15 == 0 then
		local chance = 1/(30-(functions.LuckCalc(player.Luck, 13)*2))
		if chance > rng:RandomFloat() then
...
function mod:onKnifeCollision(knife, collider)
	if functions.CheckApplyTearEffect(knife, collider) then
		local player = knife.SpawnerEntity:ToPlayer()
		local data = player:GetData()
	...
	if bomb.FrameCount == 1 and bomb.SpawnerEntity and bomb.SpawnerEntity:ToPlayer() then
		...
		if not (bomb.Variant == BombVariant.BOMB_THROWABLE and not player:HasTrinket(enums.Trinkets.BlackPepper)) then -- apply effect to throwables
			...
			if bomb.IsFetus then
				local chance = 1/(4-(functions.LuckCalc(player.Luck, 9.1, 0)*0.33)) 
				if player:HasCollectible(enums.Items.DiceBombs) and bombData.eclipsed.DiceBombs ~= false and bomb:GetDropRNG():RandomFloat() < chance then functions.InitDiceyBomb(bomb) else bombData.eclipsed.DiceBombs = false end
				if player:HasCollectible(enums.Items.GravityBombs) and bombData.eclipsed.GravityBombs ~= false and bomb:GetDropRNG():RandomFloat() < chance then functions.InitGravityBomb(bomb) else bombData.eclipsed.GravityBombs = false end
				if player:HasCollectible(enums.Items.FrostyBombs) and bombData.eclipsed.FrostyBombs ~= false and bomb:GetDropRNG():RandomFloat() < chance then functions.InitFrostyBomb(bomb) else bombData.eclipsed.FrostyBombs = false end
				if player:HasCollectible(enums.Items.BatteryBombs) and bombData.eclipsed.BatteryBombs ~= false and bomb:GetDropRNG():RandomFloat() < chance then functions.InitBatteryBomb(bomb) else bombData.eclipsed.BatteryBombs = false end
				if player:HasCollectible(enums.Items.DeadBombs) and bombData.eclipsed.DeadBombs ~= false and bomb:GetDropRNG():RandomFloat() < chance then functions.InitDeadBomb(bomb) else bombData.eclipsed.DeadBombs = false end
			end
			---SoulNadabAbihu
			...
			if player:HasCollectible(enums.Items.MirrorBombs) and not bombData.eclipsed.Mirror then
			...
			---DeadBombs
			if player:HasCollectible(enums.Items.DeadBombs) and bombData.eclipsed.DeadBombs ~= false then -- if nil or true
...
mod:AddCallback(ModCallbacks.MC_PRE_FAMILIAR_COLLISION, mod.onAbihuFamCollision, enums.Familiars.AbihuFam)
---AbihuFam UPDATE--
			...
			if sprite:IsFinished("Prize") then
				...
				if randNum <= 0.05 then --Spawn item
					...
					if mod.MongoBeggarCellReward then
						...
						Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_COLLECTIBLE, enums.Items.MongoCells, spawnpos, Vector.Zero, nil)
					end
					level:SetStateFlag(LevelStateFlag.STATE_BUM_LEFT, true)
				else
...
function mod:ShopItemCollision(pickup, collider)
	...
	if player:IsCoopGhost() then return end
	---GiftCertificate
	if player:HasTrinket(enums.Trinkets.GiftCertificate) and pickup.Price > 0 and player:GetNumCoins() >= pickup.Price then
		player:GetData().eclipsed.GiftCertificate = game:GetFrameCount()
		pickup:GetData().giftsold = true
	end
	---BlackPearl
	if player:HasTrinket(enums.Trinkets.BlackPearl) and pickup.Price < 0 then
...
function mod:PillGetEffect(pillEffect, pillColor)
	if pillColor % PillColor.PILL_GIANT_FLAG == enums.Pickups.RedPillColor then -- enums.Pickups.RedPillColorHorse
        return enums.Pickups.RedPill
    elseif pillEffect == enums.Pickups.RedPill and pillColor % PillColor.PILL_GIANT_FLAG ~= enums.Pickups.RedPillColor then
...
function mod:PillGetColor()
	for playerNum = 0, game:GetNumPlayers()-1 do
		...
		if player:HasTrinket(enums.Trinkets.Duotine) then
			return enums.Pickups.RedPillColor
		end
	end
end
...
function mod:onGetCard(rng, card, playingCards, includeRunes, onlyRunes)
	if not includeRunes and not onlyRunes then
		if (card == enums.Pickups.BannedCard and rng:RandomFloat() < 0.98) then --or card == enums.Pickups.RedPill or card == enums.Pickups.RedPillHorse then
...
function mod:onAbihuFlame(flame)
	...
	if flameData.LudovicoFire then
		...
		if flame.FrameCount % (flameData.OrbitTimer+5) == 0 and flameData.LudovicoFire > 0 then
			for _ = 1, flameData.LudovicoFire do
				local miniFlame = functions.ShootAbihuFlame(player, RandomVector()*2, flame.CollisionDamage/3, flameData.OrbitTimer, flame.Position, true)
	...
	if flameData.LeaveFlamesWhileMoving and flame.FrameCount % 7 == 0 then
		functions.ShootAbihuFlame(player, Vector.Zero, flame.CollisionDamage, math.floor(player.TearRange/4), flame.Position, true)
	end
	---boomerang
	...
	if flame:CollidesWithGrid() then
		functions.AbihuSplit(flame, player)
	end
			...
			if pickup:ToPickup() and pickup.SubType > 0 then
				...
				if not pickup:IsShopItem() and datatables.AllowedPickupVariants[pickup.Variant] then
					...
					elseif pickup.Variant == PickupVariant.PICKUP_GRAB_BAG then
						priceNum = 7
					elseif pickup.Variant == PickupVariant.PICKUP_HEART and datatables.PickupHeartPrice[pickup.SubType] then
						priceNum = 3
					end
					for _ = 1, priceNum do
	...
	for playerNum = 0, game:GetNumPlayers()-1 do
		...
		if activeItem > 0 then
			...
			elseif activeItem == enums.Items.HeartTransplant then
				if data.eclipsed.HeartTransplantUseCount then
					functions.ActiveItemText(data.eclipsed.HeartTransplantUseCount, 24, 24)
				end
			end
		end
	end
...
function mod:onPlayerRender(player) --renderOffset
	...
	if data.eclipsed and not player:IsDead() then
		...
		if Options.ChargeBars then
			local playerType = player:GetPlayerType()
			if playerType == enums.Characters.UnbiddenB and (data.eclipsed.UnbiddenFullCharge or data.eclipsed.UnbiddenSemiCharge) and not data.eclipsed.HoldingWeapon then -- and not data.TechLudo --and data.eclipsed.BlindCharacter 
				functions.RenderChargeManager(player, data.eclipsed.UnbiddenBDamageDelay, datatables.UnbiddenBData.ChargeBar, (math.floor(player.MaxFireDelay)+6))
			elseif playerType == enums.Characters.Abihu and data.eclipsed.AbihuIgnites and not player:HasCollectible(CollectibleType.COLLECTIBLE_LUDOVICO_TECHNIQUE) then
				local maxCharge = math.floor(player.MaxFireDelay)
				if not (player:HasCollectible(CollectibleType.COLLECTIBLE_SOY_MILK) or player:HasCollectible(CollectibleType.COLLECTIBLE_ALMOND_MILK)) then
					maxCharge = maxCharge + 30
				end
				functions.RenderChargeManager(player, data.eclipsed.AbihuDamageDelay, datatables.AbihuData.ChargeBar, maxCharge) --+30))
			end
...
function mod:Ignite(_, _, player)
	...
	--if player:GetPlayerType() == enums.Characters.Abihu then
		...
		--	player:TakeDamage(1, DamageFlag.DAMAGE_NO_PENALTIES | DamageFlag.DAMAGE_RED_HEARTS | DamageFlag.DAMAGE_NO_MODIFIERS, EntityRef(player), 1)
		--end
	functions.CircleSpawnX10Flame(player.Position, player)
	--end
	return true
end
mod:AddCallback(ModCallbacks.MC_USE_ITEM, mod.Ignite, enums.Items.Ignite)
...
function mod:onPickupRender(item)
	...
	for playerNum = 0, game:GetNumPlayers()-1 do
		local player = game:GetPlayer(playerNum):ToPlayer()
		if player:HasCollectible(enums.Items.TetrisDice_full) then
			...
			functions.GetIcon(filename, pos, alpha)
			break
		end
	end
end
...
function mod:BabylonCandle(_, rng, player)
	local level = game:GetLevel()
	local idx = functions.TeleportToRoom(RoomType.ROOM_PLANETARIUM, rng, true)
	if idx ~= level:GetPreviousRoomIndex() then
		game:StartRoomTransition(idx, 0, RoomTransitionAnim.FADE)
	else
		idx = functions.TeleportToRoom(RoomType.ROOM_TREASURE, rng, true)
		if idx ~= level:GetPreviousRoomIndex() then
			game:StartRoomTransition(idx, 0, RoomTransitionAnim.FADE)
			player:GetData().eclipsed.BabylonCandle = true
		else
			idx = functions.TeleportToRoom(RoomType.ROOM_TREASURE, rng)
			game:StartRoomTransition(idx, 0, RoomTransitionAnim.FADE)
			player:GetData().eclipsed.BabylonCandle = true
		end
	end
...
mod:AddCallback(ModCallbacks.MC_USE_ITEM, mod.BabylonCandle, enums.Items.BabylonCandle)
...
function mod:onBookMemoryPill(pillEffect, player, useFlag)
	...
	---PhotocopyPHD
	if player:HasTrinket(enums.Trinkets.PhotocopyPHD) then
		--[[
		local num = PillColor.NUM_STANDARD_PILLS
		for pillColor=1, num-1 do
			if itemPool:GetPillEffect(pillColor) == pillEffect then
...
function mod:RedPillUse(_, player) --, useFlag)
	local damage = 5.4
	local waves = 2
	if player:GetPill(0) == enums.Pickups.RedPillColorHorse then  --& PillColor.PILL_GIANT_FLAG == PillColor.PILL_GIANT_FLAG then --and not flags & UseFlag.USE_NOHUD > 0 then
		damage = 2 * damage
		waves = 2 * waves
	end
	functions.RedPillManager(player, damage, waves)
end
mod:AddCallback(ModCallbacks.MC_USE_PILL, mod.RedPillUse, enums.Pickups.RedPill)
...
mod:AddCallback(ModCallbacks.MC_USE_CARD, mod.CursedGem, enums.Pickups.CursedGem)
---CrystalGem
...
mod:AddCallback(ModCallbacks.MC_USE_CARD, mod.CrystalGem, enums.Pickups.CrystalGem)
---BloodyGem
...
mod:AddCallback(ModCallbacks.MC_USE_CARD, mod.BloodyGem, enums.Pickups.BloodyGem)
---LovelyGem
...
mod:AddCallback(ModCallbacks.MC_USE_CARD, mod.LovelyGem, enums.Pickups.LovelyGem)
---GoldenGem
function mod:GoldenGem(_, player)
	player:AddCoins(30)
end
mod:AddCallback(ModCallbacks.MC_USE_CARD, mod.GoldenGem, enums.Pickups.GoldenGem)
---ShinyGem
...
mod:AddCallback(ModCallbacks.MC_USE_CARD, mod.ShinyGem, enums.Pickups.ShinyGem)
---SweetGem
...
mod:AddCallback(ModCallbacks.MC_USE_CARD, mod.SweetGem, enums.Pickups.SweetGem)

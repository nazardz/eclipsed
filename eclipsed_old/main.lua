...
function EclipsedMod:onUpdateNPC(entityNPC)
	...
	if eData then 
		...
		if eData.Euthanased then 
			if entityNPC:HasMortalDamage() then
				--functions.CircleSpawn(player, 50, 0, EntityType.ENTITY_EFFECT, EffectVariant.FIRE_JET, 0)
				functions.CircleSpawnX10(entityNPC.Position, eData.Euthanased, 0, eData.Euthanased.ShotSpeed*5, EntityType.ENTITY_TEAR, TearVariant.NEEDLE, 0)
			end
			eData.Euthanased = nil
		end
		
		if eData.ArkLaserIgnore then
			eData.ArkLaserReset = eData.ArkLaserReset or 30
			eData.ArkLaserReset = eData.ArkLaserReset - 1
			if eData.ArkLaserReset <= 0 then
				eData.ArkLaserIgnore = nil
				eData.ArkLaserReset = nil
			end
		end
...
function EclipsedMod:onOblivionCard(_, player) -- card, player, useflag
	...
	local tearCard = Isaac.Spawn(EntityType.ENTITY_TEAR, TearVariant.CHAOS_CARD, 0, player.Position, vec * 14, player):ToTear() --25
	local tearCardSprite = tearCard:GetSprite()
	tearCard:GetData().OblivionTear = true
...
function EclipsedMod:useDeliObject(card, player) -- card, player, useFlag
	if datatables.DeliObject.CheckGetCard[card] then
		...
		if rng:RandomFloat() >= datatables.DeliObject.Chance then
			...
			player:AddCard(datatables.DeliObject.Variants[rng:RandomInt(#datatables.DeliObject.Variants)+1])
			--[[
...
local function AuraEnemies(ppl, auraPos, enemies, damage, range)
	local data = ppl:GetData()
	local rng = myrng
	for _, enemy in pairs(enemies) do
		...
		if ppl:HasCollectible(CollectibleType.COLLECTIBLE_EUTHANASIA) and enemy:IsActiveEnemy() then
			...
			if 1 > rng:RandomFloat() then
				enemy:GetData().Euthanased = ppl
				if enemy:IsBoss() then 
					enemy:TakeDamage(ppl.Damage*3, DamageFlag.DAMAGE_IGNORE_ARMOR, EntityRef(laser), 1)
				elseif enemy:ToNPC() then
					enemy:Kill()
					CircleSpawnX10(enemy.Position, ppl, 1, ppl.ShotSpeed*5, EntityType.ENTITY_TEAR, TearVariant.NEEDLE, 0)
				end
			end
		end
		...
		if enemy:IsVulnerableEnemy() and enemy:IsActiveEnemy() then
			...
			if ppl:HasCollectible(CollectibleType.COLLECTIBLE_JACOBS_LADDER) then
				...
				enemy:GetData().ArkLaserIgnore = 4
...
function EclipsedMod:onLaserUpdate(laser) -- low
	...
	if laser.SpawnerEntity and laser.SpawnerEntity:ToPlayer() and laser.SpawnerEntity:ToPlayer():GetPlayerType() == enums.Characters.UnbiddenB then
		...
		if not laserData.ArkLaserNext and laser.SubType ~=  LaserSubType.LASER_SUBTYPE_RING_FOLLOW_PARENT then
			laser.Color = datatables.UnbiddenBData.Stats.LASER_COLOR
		end
		
		if laserData.ArkLaserNext and laserData.ArkLaserNext.maxArk > 0 then
			--print(laserData.ArkLaserNext.pos, laserData.ArkLaserNext.range, laserData.ArkLaserNext.maxArk)
	...
	if player:HasCollectible(enums.Items.Threshold) and game:GetFrameCount()%30 == 0 then -- every 2 seconds
		for slot = 0, 3 do
			if player:GetActiveItem(slot) == enums.Items.Threshold and player:GetActiveCharge(slot) >= Isaac.GetItemConfig():GetCollectible(enums.Items.Threshold).MaxCharges then
				...
				for _, witem in pairs(itemWisps) do
					if witem.Variant == FamiliarVariant.ITEM_WISP and not functions.CheckItemType(witem.SubType) then
						if data.RenderThresholdItem and witem.SubType ~= data.RenderThresholdItem.SubType then
						end
						data.RenderThresholdItem = witem:ToFamiliar()
						witem:SetColor(Color(0,0,5,1), 30, 1, true, false)
						break
					end
				end
...
function EclipsedMod:onThreshold(_, _, player)
	...
	local data = player:GetData()
	if data.RenderThresholdItem then
		data.WispedQueue = data.WispedQueue or {}
		table.insert(data.WispedQueue, {data.RenderThresholdItem, true})
		player:AnimateCollectible(data.RenderThresholdItem.SubType)
		data.RenderThresholdItem = nil
		return false
	end
...
function EclipsedMod:onPlayerRender(player) --renderOffset
	...
	if data.RenderThresholdItem then
		...
		local wisp = data.RenderThresholdItem.SubType
